...
int Rule_explain();									// 오목의 룰 및 게임 방법을 설명하는 함수
...
int Rule_explain()  //게임의 룰 설명
{
	int x = 30, y = 5;
	char press;
	gotoxy(x, y);
	printf("*****  오목 룰 및 게임 방법 설명   *****\n");
	x = 20;
	y = 7;
	gotoxy(x, y);
	printf("* O돌이 먼저 수를 두게 됩니다 *\n");
	y = 9;
	gotoxy(x, y);
	printf("* 돌이 5개 이상 연결되면 승리합니다 *\n");
	y = 11;
	gotoxy(x, y);
	printf("* ←↑↓→ 방향키 : 바둑돌 이동, ENTER키 : 바둘돌 두기 *\n");
	y = 13;
	gotoxy(x, y);
	printf("* 게임을 시작하시려면 스페이스 바를 눌러주세요 *\n");
	y = 15;
